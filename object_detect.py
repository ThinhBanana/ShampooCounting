...
class Detection:
    ...
    def draw_bounding_boxes(self, image, boxes, polygon):
        w = image.shape[1]
        h = image.shape[0]
    ...
    def detect(self, detect_image):
        model = attempt_load(self.weights_path, device=self.device)
        ...
        image = self.draw_bounding_boxes(image, boxes, Polygon(points))
        ...
        # Post-process the output and gather bounding box coordinates, confidence scores, and class indices
        class_names = []
        for det in pred:
            if det is not None and len(det):
                det[:, :4] = det[:, :4] * np.array(shape[::-1] * 2)
                for *xyxy, conf, cls in det:
                    class_names.append(int(cls.item()))

        return np.array(image), class_names
